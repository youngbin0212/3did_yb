...
BUILD_NAME = "ut_b"    # full label written into session CSV
BUILD_TAG  = "ub"      # short suffix appended to log filenames
...
TASK_DEFS = [
    ...
    # Row 0 holds 8 LOCKED reference bricks — visible at the top of
    # the drop zone but immune to Ring (copy), Fist (delete), and
    # drag pickup. Row 1 below is where the answer slots live: the
    # user matches each pool brick to the slot directly under the
    ...
    # 5x5 starts FULLY pre-filled. The layout only lists the 8 red
    # positions, so to satisfy _check_answer the user has to Fist-
...
class App:
    ...
    MODE_BGR  = {
        ...
        "Z":    ( 95, 190,  75),
        "open": (145, 145, 145),
        # Symbolic colours kept identical across ut_a / ut_b / ut_c so
        # the same gesture-family always reads the same hue. These two
        # extras only appear in the feedback-bar wash, not as modes:
