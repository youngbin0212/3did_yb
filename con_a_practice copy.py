#!/usr/bin/env python3
"""
Gesture Workspace — BASELINE 1 PRACTICE (drag-only)
============================================
Identical to con_a.py except TASK_DEFS is replaced with two tiny practice
tasks. Goal: run a short session and confirm the metrics logger writes
non-empty CSVs covering drag actions + placement events. Mode never leaves
"open" so no action C / F / X / Z and no mode_change events are expected.
...
BUILD_NAME = "con_a_practice"   # full label written into session CSV
BUILD_TAG  = "a_practice"       # short suffix appended to log filenames
...
#  Practice mode — short tasks tuned for the drag-only baseline. There is
#  no copy / paste / delete shortcut here, so both tasks must be solvable
#  purely with pinch-and-drag. Practice 2 relies on the drag-eviction rule:
#  dropping a brick on an already-occupied slot kicks the existing brick
#  back to the pool, so wrong-colour pre-placements can still be replaced.
TASK_DEFS = [
    # Practice 1 — three empty red slots. User drags red bricks from the
    # pool into each slot. Exercises drag + placement(correct) x3.
    {
        "name": "Practice 1 - Drag Three",
    ...
    # Practice 2 — 2x2 blue with two red intruders pre-placed in the
    # wrong slots. User drags a blue brick onto each red intruder; the
    # eviction rule sends the displaced red back to the pool. Exercises
    # drag with snap-replace + placement events on intruder slots.
    {
        "name": "Practice 2 - Drag Replace",
        "file": "practice2.png",
        "cols": 2, "rows": 2,
        "layout": [
            (0, 0, "blue"), (1, 0, "blue"),
