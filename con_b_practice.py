#!/usr/bin/env python3
"""
Gesture Workspace — BASELINE 2 PRACTICE (microGEXT-style, action-based)
============================================
Identical to baseline2.py except TASK_DEFS is replaced with two tiny
practice tasks (1-3 slots each). Goal: run a short session and confirm
that pose firing + the metrics logger together produce non-empty CSVs
covering action C (copy/paste) / action X (delete) / action Z (undo)
events for the microGEXT-style pose vocabulary.

Same four poses as baseline2.py:
...
#  Practice mode — short tasks tuned to exercise the microGEXT pose
#  channel. Together they should produce action C (Ring=copy, Pinky=paste)
#  and action X (Fist=delete) events, plus drag-completion placements.
#  Open=Undo is optional during practice — fire it once to verify Z logging.
TASK_DEFS = [
    # Practice 1 — Copy & Paste, 1x3 row of red.
    # Intended flow: hover a red pool brick + Ring -> hover each empty slot
    # + Pinky three times. Exercises repeat Ring/Pinky fires.
    {
    ...
    # Practice 2 — Delete + Drag, 2x2 blue with two red intruders.
    # Intended flow: hover each red intruder + Fist (delete), then drag
    # blue bricks from the pool to fill the empty slots. (No F-equivalent
    # in baseline2, so the user identifies intruders by eye.) Optional:
    # do one Open (Undo) during the run to capture an action Z event.
    {
        "name": "Practice 2 - Delete + Drag",
        "file": "practice2.png",
        "cols": 2, "rows": 2,
        "layout": [
            (0, 0, "blue"), (1, 0, "blue"),
...
class App:
    ...
    def _fire_pose(self, pose: str, latency: float):
        ...
        if pose == "Ring":
            ...
            if obj is not None and not paste_over:
    ...
    def _draw_instruction_panel(self, frame):
        # Baseline 2: panel always shows ui_image/baseline2.png as a
        # reference for the four microGEXT poses (Ring/Pinky/Fist/Open).
        # No reveal-on-pinch behaviour and no per-mode highlighting since
        # there is no mode in this build.
