...
import atexit, copy, csv, json, math, os, random, threading, time, urllib.request
...
BUILD_NAME = "remote_c"   # full label written into session CSV
BUILD_TAG  = "rc"         # short suffix appended to log filenames

# ──────────────────────────────────────────────
#  Remote reference relay (2F Unity expert -> this 3F beginner station).
#  Unity POSTs a captured screenshot on each clap; we poll the relay for the
#  clap counter (drives which task we show) and the latest image (shown live
#  in the Instructor view). Point RELAY_URL at the machine running
#  GCT555_Server/reference_relay.py.
# ──────────────────────────────────────────────
RELAY_URL      = os.environ.get("RELAY_URL", "http://127.0.0.1:5060")
RELAY_POLL_S   = 0.7      # how often to poll the relay for new claps/images
...
SS_W   = 110
COL2_W = 200
# COL4_W (Instruction Panel + Task Workspace) trimmed a little so COL3
# (the Action Workspace) — which is the remaining width — grows to the
# right, giving a bigger drop-zone grid / bigger bricks. (remote_c)
COL4_W = 320
COL5_W = 130
COL3_W = W - (M + SS_W + G + COL2_W + G + G + COL4_W + G + COL5_W + M)  # = 588
...
SHAPE_COLORS = {
    ...
    "purple": (175,  55, 160),
    "grey":   (165, 165, 165),
    "black":  ( 45,  45,  45),   # near-black (kept slightly lifted so it
                                 # stays visible on the dark workspace bg)
...
# ──────────────────────────────────────────────
#  Tasks (remote_c): one free-form "build the structure" template, repeated
#  so each clap from the 2F expert advances to the next task. There is NO
#  fixed goal layout — the reference is the LIVE photo of whatever the expert
#  just built. A neutral snap grid (no correct colours) gives the beginner
#  alignment despite imprecise pinches. Five copy-only source bricks (one per
#  colour) sit in the Task Workspace.
# ──────────────────────────────────────────────
REMOTE_GRID_COLS, REMOTE_GRID_ROWS = 7, 5
REMOTE_SOURCE_COLORS = ["green", "red", "blue", "yellow", "black"]

def _make_remote_template():
    return {
        "name": "Build the structure",
        "file": None,                 # live image from the relay, no static file
        "cols": REMOTE_GRID_COLS, "rows": REMOTE_GRID_ROWS,
        # Full grid of snap cells. `freeform` makes them a NEUTRAL grid:
        # no goal colour shown, no auto-complete (advance is clap-driven).
        "layout": [(c, r, "grey")
                   for r in range(REMOTE_GRID_ROWS)
                   for c in range(REMOTE_GRID_COLS)],
        "freeform": True,
        "sources": REMOTE_SOURCE_COLORS,
        "full_height_workspace": True,
    }

# 10 identical templates: clap_count 1..10 maps straight to a task idx; a
# session captures up to 10 structures (clap 11+ wraps via the modulo).
TASK_DEFS = [_make_remote_template() for _ in range(10)]
...
class Shape:
    ...
    locked: bool  = False
    # Source/palette bricks (remote_c): one per colour, sit in the Task
    # Workspace. An ENDLESS palette — C-mode copy works on them AND dragging
    # one pulls out a fresh clone (the source never moves); immune to delete.
    # So the 5-colour palette always stays put no matter how it's used.
    source: bool  = False
...
class App:
    ...
    def __init__(self):
        ...
        # ── remote relay state (2F Unity expert -> here) ──
        self._remote_img       = None   # latest captured image (BGR np array)
        self._remote_lock      = threading.Lock()
        self._remote_seen_clap = 0      # clap_count last seen from the relay
        self._remote_clap      = 0      # clap_count last APPLIED on main thread
        self._remote_stop      = False
        # Per-task captured image (kept so the stage sidebar keeps a history:
        # task 1's photo doesn't vanish when task 2 arrives). task_idx -> img.
        self._stage_imgs       = {}
        self._poller = threading.Thread(target=self._poll_relay, daemon=True)
        self._poller.start()

        # ── reverse channel: push our build + stats so the 2F expert_monitor
        #    can see how the user is doing ──
        self._beg_crop        = None     # latest Action-Workspace crop (BGR)
        self._beg_lock        = threading.Lock()
        self._beg_last_stash  = 0.0
        self._task_start_wall = time.time()
        self._beg_uploader = threading.Thread(target=self._upload_beginner_loop,
                                              daemon=True)
        self._beg_uploader.start()

        # slot dimensions are recomputed per task
        self._configure_slots()
        self._load_task()

    # ── remote reference relay polling ───────
    def _poll_relay(self):
        """Background: poll the relay for the clap counter (drives the task)
        and the latest captured image (shown live in the Instructor view).
        Runs forever as a daemon; failures (server down) are retried."""
        state_url = RELAY_URL.rstrip("/") + "/state"
        img_url   = RELAY_URL.rstrip("/") + "/reference.png"
        last_ts   = None
        while not self._remote_stop:
            try:
                with urllib.request.urlopen(state_url, timeout=2) as r:
                    st = json.loads(r.read().decode("utf-8"))
                clap = int(st.get("clap_count", 0))
                ts   = st.get("ts", 0)
                if st.get("has_image") and ts != last_ts:
                    ...
                    if img is not None:
                        with self._remote_lock:
                            self._remote_img = img
                    last_ts = ts
                # main thread compares _remote_seen_clap vs _remote_clap and
                # advances the task; just record the latest here.
                self._remote_seen_clap = clap
            except Exception:
                pass   # relay unreachable — keep retrying
            time.sleep(RELAY_POLL_S)

    def _remote_advance(self, clap_count):
        """Apply a new clap from the expert: jump to task N (clap 1 -> task 1)
        and reset the workspace to the fresh colour palette."""
        self._prev_task_idx = self.task_idx
        self.task_idx   = max(0, clap_count - 1)
        # Save this clap's capture under its task index so the stage sidebar
        # keeps every structure's photo (doesn't vanish on the next clap).
        with self._remote_lock:
            img = self._remote_img
        if img is not None:
            self._stage_imgs[self.task_idx % len(self.tasks)] = img.copy()
        self._correct_t = 0.0
        self._load_task()
        self._notify(f"Task {clap_count}", kind="task")

    # ── reverse channel: push our build to the expert monitor ──
    def _stash_build(self, canvas):
        """Called from the render loop: stash a copy of the Action-Workspace
        region (the user's build) for the uploader thread. Throttled."""
        now = time.time()
        if now - self._beg_last_stash < 0.30:
            return
        self._beg_last_stash = now
        crop = canvas[AW_Y:AW_Y + AW_H, AW_X:AW_X + AW_W].copy()
        with self._beg_lock:
            self._beg_crop = crop

    def _upload_beginner_loop(self):
        """Background: POST the build image + live stats to the relay so the
        2F expert_monitor can watch the user's progress."""
        url = RELAY_URL.rstrip("/") + "/beginner"
        while not self._remote_stop:
            time.sleep(0.4)
            with self._beg_lock:
                crop = self._beg_crop
            ...
            try:
                ...
                if not ok:
                    continue
                # self view: the user's webcam, downscaled, so the expert can
                # see the person doing the gestures.
                self_png = None
                ...
                if raw is not None:
                    ...
                    if sw > 360:
                        sc = 360.0 / sw
                        raw = cv2.resize(raw, (360, max(1, int(sh * sc))))
                    ok2, sbuf = cv2.imencode(".png", raw)
                    if ok2:
                        self_png = sbuf.tobytes()
                placed = 0
                colors = {}
                for s in self.shapes:
                    if s.alive and s.slot is not None and not s.source:
                        placed += 1
                        colors[s.kind] = colors.get(s.kind, 0) + 1
                self._post_beginner(
                    url, buf.tobytes(),
                    task=self.task_idx + 1,
                    placed=placed,
                    elapsed=time.time() - self._task_start_wall,
                    colors=colors,
                    self_png=self_png,
                )
            except Exception:
                pass   # relay unreachable — keep trying

    @staticmethod
    def _post_beginner(url, png, task, placed, elapsed, colors, self_png=None):
        boundary = "----begboundary"
        parts = []
        ...
        field("task", task)
        field("placed", placed)
        field("elapsed", round(elapsed, 1))
        field("colors", json.dumps(colors))
        image("image", "build.png", png)
        if self_png is not None:
            image("selfview", "self.png", self_png)
        parts.append(f"--{boundary}--\r\n".encode())
        body = b"".join(parts)
    ...
    def _load_tasks(self):
        ...
        for d in TASK_DEFS:
            ...
            if fname:
                ...
                # remote_c: reference comes live from the relay, no static file
                t["image"] = None
            result.append(t)
        return result
    ...
    def _load_task(self):
        ...
        self._configure_slots()
        self._task_start_wall = time.time()   # for the build-uploader elapsed
        ...
        # remote_c free-form task: just lay out the copy-only colour palette
        # in the Task Workspace; the grid is neutral (no goal) and there is
        # no pool / pre-placed / auto-delete to set up.
        if t.get("freeform"):
            self._place_sources(t.get("sources", []))
            self.metrics.start_task(self.task_idx, t["name"], len(t["layout"]))
            return
    ...
    def _place_sources(self, colors):
        """remote_c: lay out one COPY-ONLY source brick per colour across the
        top of the Task Workspace. They're a fixed palette (source=True) the
        user copies from with C-mode; immune to drag/delete so they stay put."""
        n = len(colors)
        if n == 0:
            return
        # 2 per row (A B / C D / E), spaced ~half a brick apart, centred in
        # the Task Workspace, kept above the drop-zone bottom line.
        cx_mid = TW_X + TW_W / 2.0
        gap_x  = self.brick_w * 0.9
        gap_y  = self.brick_h * 0.7
        col_x  = [cx_mid - (self.brick_w + gap_x) / 2.0,
                  cx_mid + (self.brick_w + gap_x) / 2.0]
        top_y    = TW_Y + TW_HEADER_H + self.brick_h / 2.0 + 14
        row_step = self.brick_h + gap_y
        n_rows   = (n + 1) // 2
        dz_bottom = self.dz_y + self.dz_h
        last_y = top_y + (n_rows - 1) * row_step + self.brick_h / 2.0
        if n_rows > 1 and last_y > dz_bottom - 6:
            row_step = max(self.brick_h * 0.9,
                           ((dz_bottom - 6) - top_y - self.brick_h / 2.0)
                           / (n_rows - 1))
        labels = "ABCDEFGHIJKLMNOPQRSTUVWXY"
        for i, kind in enumerate(colors):
            row, colp = divmod(i, 2)
            lone = (i == n - 1 and n % 2 == 1)      # last odd one (E) centred
            cx = cx_mid if lone else col_x[colp]
    ...
    def _slot_expected(self, slot):
        ...
        Free-form remote tasks use a neutral grid, so this is always None."""
        t = self.tasks[self.task_idx % len(self.tasks)]
        if t.get("freeform"):
            return None
    ...
    def _check_answer(self) -> bool:
        t = self.tasks[self.task_idx % len(self.tasks)]
        # remote_c: free-form build has no programmatic goal; the task only
        # advances when the 2F expert claps again (handled by the relay poll).
        if t.get("freeform"):
            return False
    ...
    def _execute(self):
        ...
        if self.mode == "C":
            ...
            if obj is not None and obj.locked:
                ...
                # Copies of a palette source paste as NORMAL bricks (movable,
                # deletable) — only the original source stays fixed.
                self.clipboard.source = False
                self.clipboard.locked = False
            ...
            if obj:
                ...
                if obj.locked or obj.source:
                    self._notify("Source: can't delete", kind="warn")
    ...
    def update(self, result, raw_frame):
        ...
        # The 2F expert clapped: advance to the next task (reset workspace).
        # Done on the main thread so it never races the render/poller.
        if self._remote_seen_clap != self._remote_clap:
            self._remote_clap = self._remote_seen_clap
            self._remote_advance(self._remote_clap)
            ...
            if self.mode == "open":
                if new_pinch:
                    if not self._prev_pin:
                        ...
                        if hit and hit.source:
                            # ENDLESS palette: dragging a source never moves it
                            # — pull out a fresh NORMAL clone and drag THAT, so
                            # the source stays put and supplies infinite bricks
                            # (C-mode copy works on it too). Snapshot first so
                            # undo drops the clone.
                            self._push_history()
                            clone = Shape(
                                id    = self._next_id,
                                px    = hit.px, py = hit.py,
    ...
    def _draw_stage_sidebar(self, frame):
        ...
        for i in range(n):
            ...
            # remote_c: each clap's captured photo, kept per task so the
            # history stays visible after advancing.
            img = self._stage_imgs.get(i)
            label_band_h = 22
    ...
    def _draw_reference(self, frame):
        ...
        # remote_c: the reference is the LIVE photo the 2F expert captured,
        # pushed through the relay and refreshed by the background poller.
        with self._remote_lock:
            img = self._remote_img
        if img is not None:
            sh, sw = img.shape[:2]
            ...
            # No capture received from the 2F expert station yet.
            self._put(frame, "Waiting for capture...",
                      RF_X + 12, img_top + 34, 0.46, (95, 95, 90))
            self._put(frame, "(expert: build + clap)",
                      RF_X + 12, img_top + 58, 0.42, (110, 110, 105))

        # how many bricks the user has placed on the grid (sources excluded)
        placed_n = sum(1 for s in self.shapes
                       if s.alive and s.slot is not None and not s.source)
        self._put(frame, f"{placed_n} placed", RF_X + 12, RF_Y + RF_H - 14,
                  0.42, (95, 95, 90))
    ...
    def _draw_instruction_panel(self, frame):
        ...
        # remote_c: the gesture shortcuts are ALWAYS visible (no pinch-to-
        # reveal), so the beginner can always see the C / F / X / Z guide.
    ...
    def _draw_action_workspace(self, frame):
        ...
        t = self.tasks[self.task_idx % len(self.tasks)]
        freeform = t.get("freeform")
        for col, row, expected in t["layout"]:
            x1, y1, x2, y2 = self._slot_rect(col, row)
            occ = next((s for s in self.shapes
                        if s.alive and s.slot == (col, row)), None)
            if freeform:
                # remote_c neutral snap grid: no goal colour / correctness,
                # just a faint cell outline plus the drag/paste snap target.
                if paste_preview == (col, row):
                    bc, bw = (60, 220, 245), 3          # yellow snap target
                elif drag_preview == (col, row):
                    bc, bw = (75, 235, 230), 3          # cyan snap target
                elif occ is not None:
                    bc, bw = (120, 140, 120), 2         # occupied cell
                else:
                    bc, bw = (95, 95, 90), 1            # empty grid line
    ...
    def _draw_shapes(self, frame):
        for s in self.shapes:
            ...
            # ENDLESS palette source: draw a little offset "stack" behind it
            # so it reads as an infinite pile you copy / drag clones from.
            if s.source:
                for k in (2, 1):
                    off = 5 * k
...
def main():
    ...
    with mp_vision.HandLandmarker.create_from_options(options) as landmarker:
        while cap.isOpened():
            ...
            app._stash_build(canvas)   # share the build with expert_monitor
