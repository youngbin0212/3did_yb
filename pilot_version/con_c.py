...
BUILD_NAME = "con_c"   # full label written into session CSV
BUILD_TAG  = "c"       # short suffix appended to log filenames
...
class App:
    ...
    def _draw_action_workspace(self, frame):
        ...
        for col, row, expected in t["layout"]:
            ...
            if not correct:
                # Goal swatch used to fill at 0.10 alpha — so washed-out
                # against the full-opacity bricks that users couldn't tell
                # the target hue actually matched the brick. Bump the fill
                # and, for a plain empty slot, draw the border in the goal
                # colour at full saturation so the target hue reads
                # identically to a brick.
