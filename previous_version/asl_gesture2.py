...
class App:
    ...
    def __init__(self):
        ...
        self.dragging:      Optional[SceneObj] = None   # 드래그 중인 객체
        self._drag_offset:  tuple = (0.0, 0.0)          # 핀치 지점과 객체 중심의 offset
        self.r_ptr:         Optional[tuple] = None
        self._r_ptr_stable: Optional[tuple] = None
        self.r_pinch        = False
            ...
            if self.mode == "open":
                if new_pinch:
                    if not self._prev_pin:
                        ...
                        if hit:
                            self.dragging = hit
                            self._drag_offset = (hit.cx - mx, hit.cy - my)
                    if self.dragging:
                        # 핀치 유지 → 객체 위치 이동
                        self.dragging.cx = mx + self._drag_offset[0]
                        self.dragging.cy = my + self._drag_offset[1]
                else:
                    # 핀치 해제 → 드롭
                    if self.dragging:
                        self._notify(f"Moved: {self.dragging.label}")
                    self.dragging = None
    ...
    def draw(self, frame: np.ndarray) -> None:
        ...
        for o in self.objects:
            ...
            is_drag = bool(self.dragging and self.dragging.id == o.id)

            a  = 0.65 if is_drag else (0.45 if is_hov else (0.25 if is_hl else 0.12))
            ov = frame.copy()
            cv2.rectangle(ov, (x1,y1), (x2,y2), o.color, -1)
            cv2.addWeighted(ov, a, frame, 1-a, 0, frame)

            bc = o.color if (is_drag or is_hov or is_hl) else (100, 105, 115)
            bw = 3 if is_drag else (2 if (is_hov or is_hl) else 1)
            cv2.rectangle(frame, (x1,y1), (x2,y2), bc, bw)
            # 드래그 중이면 점선 느낌의 코너 마커 추가
            if is_drag:
                m = 10
                for (ex, ey) in [(x1,y1),(x2,y1),(x1,y2),(x2,y2)]:
                    sx = 1 if ex == x1 else -1
                    sy = 1 if ey == y1 else -1
                    cv2.line(frame, (ex, ey), (ex + sx*m, ey), o.color, 2)
                    cv2.line(frame, (ex, ey), (ex, ey + sy*m), o.color, 2)
            tc = o.color if (is_drag or is_hov or is_hl) else (190, 195, 205)
        ...
        # legend
        legend = [
            "open palm + pinch  -> drag",
            "C + object         -> copy",
            "C + empty          -> paste",
            "F + object         -> highlight",
            "thumbs down + obj  -> delete",
            "Z (index up)       -> undo",
        ]
